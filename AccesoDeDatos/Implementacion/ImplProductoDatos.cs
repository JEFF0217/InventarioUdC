...
namespace AccesoDeDatos.Implementacion
{
    public class ImplproductoDatos
    {
        ...
        public IEnumerable<tb_producto> ListarRegistros(string filtro, int paginaActual, int numRegistrosPorPagina, out int totalRegistros)
        {
            var lista = new List<tb_producto>();
            using (InventarioUdCDBEntities bd = new InventarioUdCDBEntities())
            {
                int regDescartados = (paginaActual - 1) * numRegistrosPorPagina;
                lista = (from m in bd.tb_producto
        ...
        public bool GuardarRegistro(tb_producto registro)
        {
            try
            {
                using (InventarioUdCDBEntities bd = new InventarioUdCDBEntities())
        ...
        public tb_producto BuscarRegistro(int id)
        {
            using (InventarioUdCDBEntities bd = new InventarioUdCDBEntities())
            {
        ...
        public bool EditarRegistro(tb_producto registro)
        {
            try
            {
                using (InventarioUdCDBEntities bd = new InventarioUdCDBEntities())
